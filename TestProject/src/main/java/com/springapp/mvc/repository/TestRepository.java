...
public class TestRepository {
    ...
    public List<QuestionsEntity> getQuestionsByTestPath(String path) {
        return session.getCurrentSession().createSQLQuery("SELECT questions.* FROM tests left join test_questions on tests.ID=test_questions.ID_test left join questions on  test_questions.ID_question=questions.ID where tests.pathtotest=:path ORDER BY ID_question").addEntity(QuestionsEntity.class).setString("path", path).list();
    }
    ...
    public List getResult(int idform) {
        ...
        sql.append("        (SELECT \n");
        sql.append("        *,\n");
        sql.append("            IF(correct = 1 AND usersID IS NOT NULL and ID_answer<>0\n");
        sql.append("                AND type_question < 3, 1, 0) AS userAnswerCorrect\n");
        sql.append("    FROM\n");
        sql.append("        (SELECT \n");
        sql.append("        test.*, answers_user.textanswer, answers_user.ID AS usersID, answers_user.ID_answer\n");
        sql.append("    FROM\n");
        sql.append("        (SELECT \n");
        ...
        sql.append("    GROUP BY ID) AS finish left join questions on finish.ID=questions.ID order by finish.ID");
    ...
    public List getAllResults() {
        ...
        sql.append("SELECT users.name, users.surname, users.email, users.phone, tests.testname, forms.datestart,forms.datefinish, forms.ID, forms.quantity_question, forms.correct_question,forms.quantity_question-forms.correct_question as uncorrect, all_ID, correct, QuantityForm from (SELECT COUNT(ID_form) as QuantityForm, ID_form, GROUP_CONCAT(ID ORDER by ID SEPARATOR ':') as all_ID, GROUP_CONCAT(correct ORDER BY ID SEPARATOR ':') as correct from (SELECT\n");
        sql.append("        ID_form,\n");
        sql.append("        ID,\n");
        sql.append("        IF(cor = usercorrect AND cor <> 0, 1, 0) AS correct\n");
        sql.append("      FROM\n");
        ...
        sql.append("                 (SELECT\n");
        sql.append("                    test.*,\n");
        sql.append("                    answers_user.ID_form,\n");
        sql.append("                    answers_user.textanswer,\n");
        sql.append("                    answers_user.ID AS usersID,\n");
        sql.append("                    answers_user.ID_answer\n");
        sql.append("                  FROM\n");
        sql.append("                    (SELECT\n");
        sql.append("                       questions.ID,\n");
        sql.append("                       questions.type_question,\n");
        ...
        sql.append("                       LEFT JOIN answers ON questions.ID = answers.ID_question\n");
        sql.append("                     WHERE\n");
        sql.append("                       questions.ID IN (SELECT answers_user.ID_question\n");
        sql.append("                                        FROM\n");
        sql.append("                                          answers_user\n");
        sql.append("                                        GROUP BY answers_user.ID_question\n");
        sql.append("                       )) AS test LEFT JOIN answers_user ON test.ID = answers_user.ID_question\n");
        sql.append("                                                            AND (test.idanswer = answers_user.ID_answer OR\n");
        sql.append("                                                                 answers_user.ID_answer =\n");
        sql.append("                                                                 0)) AS main) AS ourquery) AS repeatQ\n");
        sql.append("         GROUP BY ID, ID_form) AS finish\n");
        sql.append("WHERE NOT finish.ID_form IS NULL) as end\n");
        sql.append("GROUP BY ID_form) as end left join forms on ID_form=forms.ID LEFT JOIN users on forms.ID_user = users.ID LEFT JOIN tests on forms.ID_test = tests.ID ORDER BY QuantityForm desc\n");
        SQLQuery query = session.getCurrentSession().createSQLQuery(sql.toString());
        query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
    ...
    public List getListTestByKey(int category, String keySearch) {
        ...
        query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
        if (category != 0) {
            query.setInteger("idcategory", category);
        }
